...
def pretrain_one_epoch(
    ...
    for i, entry in enumerate(pbar):
        ...
        #     global_feats[2::3], local_feats[2::3], local_mask[2::3], scales[2::3], positions[2::3])
        # logits = torch.cat([p_logits, n_logits], 0)
        outputs = model(None, local_feats, local_mask, scales, positions, None, None, None, None, None, as_pretrained=True)

        if type(class_loss) == nn.MSELoss:
            loss = class_loss(outputs, global_feats).mean()
        elif type(class_loss) == BinaryCrossEntropyWithLogits:
            target = (global_feats > 0) * 1.0
            loss = class_loss(outputs, target).mean()
        ...
        if not (i + 1) % freq:
            step = epoch + i / loader_length
            print("loss:", train_losses.last_avg.item())
            # print('step/loss/lr: {:.3f}, {:.9f}, {}'.format(step, train_losses.last_avg.item(), scheduler[0].get_last_lr()[0]))
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        evaluate_function = partial(mean_average_precision_revisited_rerank, model=model, cache_nn_inds=cache_nn_inds,
            ...
            ks=recall, gnd=query_loader.dataset.gnd_data, output_path=output_path
        )
