...
def mean_average_precision_revisited_rerank(
    ...
    ks: List[int],
    gnd, output_path=".") -> Dict[str, float]:
    ...
    pickle_save(osp.join(output_path, 'medium_nn_inds.pkl'), ranks.T)
    ...
    # pickle_save('nn_inds_rerank.pkl', closest_indicess)
    # pickle_save('nn_dists_rerank.pkl', closest_dists)

    ranks = deepcopy(hard_nn_inds)
    ranks[:, :top_k] = deepcopy(closest_indices)
    ranks = ranks.cpu().data.numpy().T
    pickle_save(osp.join(output_path, 'hard_nn_inds.pkl'), ranks.T)
    ...
    json_save(osp.join(output_path, 'eval_revisited.json'), out)
    return out
