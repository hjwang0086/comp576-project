...
class FeatureDataset(Dataset):
    ...
    def __getitem__(self, index, load_image=False):
        ...
        Output
            global_desc: (2048, )
            local_desc: (max_sequence_len, d_local)
        ...
        assert osp.exists(global_path), f"Not found global path \'{global_path}\'"
        assert osp.exists(local_path), f"Not found local path \'{local_path}\'"
        global_desc = datum_io.ReadFromFile(global_path)
        locations, scales, desc, attention, _ = feature_io.ReadFromFile(local_path)

        # TODO: no error during the 1st run (worst case), may require extra input
        self.d_local = max(self.d_model, desc.shape[-1])
        assert self.d_local != 0, "Loading empty local feature at first (worst case)"

        local_mask = torch.ones(self.max_sequence_len, dtype=torch.bool)
        local_desc = np.zeros((self.max_sequence_len, self.d_local), dtype=np.float32)
        scale_inds = torch.zeros(self.max_sequence_len).long()
        seq_len = min(desc.shape[0], self.max_sequence_len)

        # debug: using unconverged 0_baseline causes ValueError since desc may be empty
        try:
            local_desc[:seq_len] = desc[:seq_len]
        except ValueError:
            pass # just don't modify anything if desc is empty
        ...
        except IndexError: # when empty, "IndexError: too many indices for array: array is 1-dimensional, but 2 were indexed"
            pass
