...
from utils.training import pretrain_one_epoch
...
def main(epochs, cpu, cudnn_flag, visdom_port, visdom_freq, temp_dir, seed, no_bias_decay, max_norm, resume):
    ...
    class_loss = get_loss()
    print(f"Using class loss {class_loss}")
    ...
    for epoch in range(epochs):
        ...
        pretrain_one_epoch(model=model, loader=loaders.train, class_loss=class_loss, optimizer=optimizer, scheduler=scheduler, max_norm=max_norm, epoch=epoch, freq=visdom_freq, ex=ex)
        ...
        torch.save({'state': deepcopy(model.state_dict()), 'optim': optimizer.state_dict()}, save_name)
