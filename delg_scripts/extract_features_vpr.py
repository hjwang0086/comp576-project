...
flags.DEFINE_string(
    'model_path', '/tmp/r50delg_gld',
    'Directory where the model path is stored.')
flags.DEFINE_boolean( # https://github.com/tensorflow/models/blob/master/research/delf/delf/protos/delf_config.proto
    'is_tf2_exported', False,
...
flags.DEFINE_enum('file_ext', 'jpg', ['jpg', 'png'],
      'File extension for files.')
flags.DEFINE_boolean( # https://github.com/tensorflow/models/blob/master/research/delf/delf/protos/delf_config.proto
    'renamed', False,
    'Whether query file is renamed (used for Pitts30k).')
...
def main(argv):
  ...
  mat = loadmat(FLAGS.dataset_file_path)
  matStruct = mat['dbStruct'].item()

  if FLAGS.image_set == "query":
    image_list = [f[0].item() for f in matStruct[3]]
  elif FLAGS.image_set == "index": # convert to png...
    image_list = [f[0].item() for f in matStruct[1]]

  if FLAGS.renamed:
    new_image_list = []
    for img_name in image_list:
      img_name = img_name.split("/")
      img_name = img_name[:-1] + ["query_" + img_name[-1]]
      img_name = "/".join(img_name)
      new_image_list.append(img_name)
    image_list = new_image_list

  image_list = [os.path.splitext(f)[0] + f".{FLAGS.file_ext}" for f in image_list]
  ...
  config.is_tf2_exported = FLAGS.is_tf2_exported
  ...
  for i in range(num_images):
    ...
    # pitts30k has overlap image name, should separate
